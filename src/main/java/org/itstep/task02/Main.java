 ...
 * 3) Добавьте защищенное поле налога (tax)
 * 4) Создайте геттер для tax
 * 5) Переопределите метод setPaymentType(), который задает налог в зависимости от вида оплаты
 * 6) Переопределите метод getPayment(), который должен возвращать К оплате (т.е. сумму с учетом налогов)
 * 7) Переопределите метод toString(), который должен возвращать строку вида: ФИО | Налог, % | Сумма | К оплате
 * 8) Раскомментируйте код в методе main()
 *
